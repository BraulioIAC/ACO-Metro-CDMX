...
myFont1 = pygame.font.Font(None, 40)
myFont2 = pygame.font.Font(None, 20)
myFont3 = pygame.font.Font(None, 30)
myFont4 = pygame.font.Font(None, 35)
...
def CrearMetro():
    ...
    inst = myFont1.render(('Instrucciones:'),0,BLUE)
    screen.blit(inst,(10,10))
    
    inst = myFont3.render(('1.-Seleccione con el ratón el nodo de inicio.'),0,BLACK)
    screen.blit(inst,(10,60))
    inst = myFont3.render(('2.-Seleccione con el ratón el nodo de fin.'),0,BLACK)
    screen.blit(inst,(10,90))
    inst = myFont3.render(('3.-Tecla ESPACIO para realizar una nueva búsqueda.'),0,BLACK)
    screen.blit(inst,(10,120))
    
    inst = myFont4.render(('Desde: '),0,BLUE)
    screen.blit(inst,(800,60))
    inst = myFont4.render(('Hasta: '),0,BLUE)
    screen.blit(inst,(800,100))
    ...
    for i in range(len(nodosMapa)):
        ...
        if (i == 14):
            miTexto = myFont2.render((estaciones[i][0]),0,BLACK)
            screen.blit(miTexto,(nodosMapa[i,0]-25,nodosMapa[i,1]+10))
        else:
            miTexto = myFont2.render((estaciones[i][0]),0,BLACK)
            screen.blit(miTexto,(nodosMapa[i,0]-25,nodosMapa[i,1]-20))
    pygame.draw.line( screen, BLUE, (0, SQUARE_SIZE), (WIDTH, SQUARE_SIZE), LINE_WIDTH )
    
    return

def AntsMetro (inicio1, fin1):
...
def pintaRuta(rutaMinFinal):
    for i in range(len(rutaMinFinal)):
        ...
        if (i1 == 810 and i2 == 553):
            miTexto = myFont2.render((estaciones[rutaMinFinal[i][0]][0]),0,RED)
            screen.blit(miTexto,(i1-25,i2+10))
        elif (f1 == 810 and f2 == 553):
            miTexto = myFont2.render((estaciones[rutaMinFinal[i][1]][0]),0,RED)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if (flag == 0):
                ...
                desde = myFont3.render((estaciones[inicio][0]),0,BLACK)
                screen.blit(desde,(900,60))
                siCalcula += 1
                flag = 1
                
            else:
                fin = selectNodo (mouse_x,mouse_y)
                hasta = myFont3.render((estaciones[fin][0]),0,BLACK)
                screen.blit(hasta,(900,100))
                siCalcula += 1
                flag = 0
